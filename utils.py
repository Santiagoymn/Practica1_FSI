...
class AR(Queue):
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A = sorted(self.A, reverse=True, key=lambda Node: Node.path_cost) #Node.path_cost + heristica
...
class AR_heuristica(Queue):
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A.sort(reverse=True, key=lambda Node: (Node.path_cost+self.problem.h(Node)))  #Node.path_cost + heuristica
